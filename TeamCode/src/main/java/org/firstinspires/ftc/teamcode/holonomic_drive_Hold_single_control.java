   ...
   Robot wheel mapping:
      ...
      X  BL       BR  X
        X           X
          X       X
*/
@TeleOp(name = "SharkTeleOpHold-single-control", group = "competition")
//@Disabled
public class holonomic_drive_Hold_single_control extends OpMode {
     ...
     * Constructor
     */
    public holonomic_drive_Hold_single_control() {
    }
        ...
        public void init() {
            ...
            public void loop() {
                ...
                if (gamepad1.left_trigger > 0.0 && shark.motorLift.getCurrentPosition() > armMinPos) // encoder greater that lower limit
                {
                    shark.motorLift.setPower(-gamepad1.left_trigger ); // let trigger run -motor DOWN / div in half to slow motor
                    armHoldPosition = shark.motorLift.getCurrentPosition(); // while the lift is moving, continuously reset the arm holding position
                }
                else if (gamepad1.right_trigger > 0.0 && shark.motorLift.getCurrentPosition() < armMaxPos) //encoder less than Max limit
                {
                    shark.motorLift.setPower(gamepad1.right_trigger); //let trigger run +motor UP
                    armHoldPosition = shark.motorLift.getCurrentPosition(); // while the lift is moving, continuously reset the arm holding position
                }
