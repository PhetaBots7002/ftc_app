...
public class TestHardwareTeleOp extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        r.init(hardwareMap); //initializes hardware mapping from remote hardware configuration
        ...
        while (opModeIsActive()) {  // run until the end of the match (driver presses STOP)
            ...
            r.motorLeft.setPower(gamepad1.left_stick_y);
            r.motorRight.setPower(gamepad1.right_stick_y);

            // Arm Control - Uses dual buttons to control motor direction
            if(gamepad1.right_bumper) {
                r.motorArm.setPower(-gamepad1.right_trigger); // if both Bumper + Trigger, then negative power, runs arm down
            }
            else {
                r.motorArm.setPower(gamepad1.right_trigger);  // else trigger positive value, runs arm up
            }
/*
            //servo commands
            if(gamepad1.a) {
                r.servoHandR.setPosition(r.OPEN);  // Note: to change position value, go to HardwareSetup, or create new value or variable in this OpMode
                r.servoHandL.setPosition(r.OPEN);
            }
            else if (gamepad1.b) {
                r.servoHandR.setPosition(r.CLOSED);
                r.servoHandL.setPosition(r.CLOSED);
            }
*/
